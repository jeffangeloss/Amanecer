...
int main()
{
    ...
    for(int i=0;i<numDIAS;i++)
    {
        ...
        minB = salario_B[i]<minB? salario_B[i] : minB ;
        maxB = salario_B[i]>maxB? salario_B[i] : maxB ;
        promB += salario_B[i]/(float)numDIAS ;

        minC = salario_C[i]<minC? salario_C[i] : minC ;
        maxC = salario_C[i]>maxC? salario_C[i] : maxC ;
